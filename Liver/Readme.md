...
## 使用步骤
1. 运行 `Check_Meta.py`：检查数据完整性并统计
2. 运行 `Get_trainData.py`：生成训练/推理用数据
3. 运行任一模型脚本（如 `V3_P6.py`）：调用 LLM 生成报告（需先设置环境变量 `SILICONFLOW_KEY`）
4. 运行 `Scoring.py`：批量评分，输出各模型各提示词的评分 CSV

## 提示词说明
- `prompt5` 与 `prompt2` 比较，用于对比样本量（few-shot 样例数）对结果的影响
...
论文最终采用的提示词选择顺序：P7 → P4 → P8 → P6 → P9 → P10 → P11
