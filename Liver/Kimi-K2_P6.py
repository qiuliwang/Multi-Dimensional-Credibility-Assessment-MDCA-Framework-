...
MAX_RETRIES = 3
RETRY_DELAY = 5  # 秒
MAX_FILES = 17500  # 总处理文件数上限，防止误操作

BATCH_SIZE = 2000  # 每批最多处理文件数
BATCH_SIZE = MAX_FILES  # 每批最多处理文件数
...
OUTPUT_DIR = fr"E:\Data\Reports_Collected\AI_Conclusion\KIMI\{PROMPT_NAME}\{BATCH_NAME}"
...
def generate_report(rel_path, system_message):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            # 改成：把 enable_thinking 放在“顶层”，其余参数不变
            if MODEL_NAME in MODELS_DISABLE_THINKING:
...
if __name__ == "__main__":
    ...
    all_files = gather_input_files(INPUT_DIR)[:MAX_FILES]
